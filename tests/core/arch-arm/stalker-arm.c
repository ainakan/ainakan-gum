                           ...
                           TestArmStalkerFixture * fixture)
{
  ...
  target->thread = g_thread_new("async-task-3",
...
TESTCASE (follow_thread)
{
  ...
  thread = g_thread_new("pool-7-thread-11", run_stalked_briefly, &channel);
...
TESTCASE (unfollow_should_handle_terminated_thread)
{
  ...
  for (i = 0; i != 10; i++)
  {
    ...
    thread = g_thread_new("bg-worker-34", run_stalked_into_termination,
        &channel);
    thread_id = sdc_await_thread_id (&channel);
                                   ...
                                   GumThreadId * thread_id)
{
  ...
  thread = g_thread_new("worker-11", sleeping_dummy, &sync_data);
